...
class Parser {
    ...
    private static class SymbolTree {
        ...
        SymbolTree(Element sym, Tree tree) {
            this.sym = sym;
            this.tree = tree;
        }
    ...
    private static class Dual {
        ...
        Dual(SymbolTree lhs, Symbol a, Symbol b) {
            this.lhs = lhs;
            this.a = a;
            this.b = b;
        }
    ...
    private boolean allSymsInSet(Element[] rhs, Set query) {
        for (int i = 0; i < rhs.length; i++) {
            if (rhs[i] instanceof Symbol
    ...
    private Grammar computeNoEpsilons(Grammar g, HashMap orig_rules) {
        ...
        while (changed) {
            ...
            while (it.hasNext()) {
                Grammar.Rule rule = (Grammar.Rule) it.next();
                Symbol lhs = rule.getLeftSide();
                if (nullable.get(lhs) != null) {
                    continue;
                }
                Element[] rhs = rule.getRightSide();
                boolean can_null = true;
                for (int i = 0; can_null && i < rhs.length; i++) {
                    can_null = rhs[i] instanceof Symbol
    ...
    private Tree treeFor(Grammar.Rule rule, HashMap tree_map) {
        ...
        for (int i = 0; i < children.length; i++) {
            if (rhs[i] instanceof Symbol && tree_map != null) {
    ...
    private static class AddNonNullData {
        ...
        AddNonNullData(Grammar dest, Symbol lhs,
                Element[] rhs, Tree[] known,
    ...
    private void addNonNullRules(AddNonNullData data, int pos) {
        if (pos == data.rhs.length) {
            ...
            if (len != 0) {
                Element[] new_rhs = new Element[len];
                int j = 0;
                for (int i = 0; i < data.rhs.length; i++) {
                    if (data.known[i] == null) {
                        new_rhs[j++] = data.rhs[i];
                    }
                }
    ...
    private Grammar computeNoUnit(Grammar g, HashMap rule_orig) {
        ...
        while (it.hasNext()) {
            Grammar.Rule rule = (Grammar.Rule) it.next();
            Element[] rhs = rule.getRightSide();
            if (rhs.length == 1 && rhs[0] instanceof Symbol) {
                Symbol lhs = rule.getLeftSide();
                Symbol a = (Symbol) rhs[0];
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                SymbolTree b = (SymbolTree) it2.next();
                Iterator it3 = g.getRules((Symbol) b.sym).iterator();
                while (it3.hasNext()) {
                    Grammar.Rule rule = (Grammar.Rule) it3.next();
                    Element[] rhs = rule.getRightSide();
                    if (rhs.length != 1
                            || !(rhs[0] instanceof Symbol)) {
    ...
    private void computeChomsky(Grammar g) {
        ...
        while (it.hasNext()) {
            ...
            if (rhs.length == 1) {
                ...
                Symbol a = symFor(rhs[0], smap);
                addDual(lhs, a, symFor(rhs[1], smap), t);
            }
        }
    }

    private HashMap getSingles(Element e) {
    ...
    private Symbol symFor(Element e, HashMap smap) {
        if (e instanceof Symbol) {
            return (Symbol) e;
        }
        Symbol ret = (Symbol) smap.get(e);
        if (ret == null) {
            ret = new Symbol();
    ...
    private void addDual(Symbol lhs, Symbol a,
            Symbol b, Tree t) {
